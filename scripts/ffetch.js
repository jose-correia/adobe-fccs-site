...
function endIteration(context) {
  const { lastIndex, lastEntry, lastRecord = () => {} } = context;
  lastRecord(lastIndex, lastEntry);
}
...
async function* request(url, context) {
  ...
  context.lastIndex = offsetParam;
  const maxOffset = offsetParam + chunks * chunkLimit;
  for (
    let offset = offsetParam, total = Infinity;
    offset < total && offset < maxOffset;
    offset += chunks
  ) {
...
function offset(upstream, context, offset) {
  context.offset = offset;
  return upstream;
}

function lastRecord(upstream, context, lastRecord) {
  context.lastRecord = lastRecord;
  return upstream;
}
...
function assignOperations(generator, context) {
  ...
  const functions = {
    ...
    offset: offset.bind(null, generator, context),
    lastRecord: lastRecord.bind(null, generator, context),
  };
